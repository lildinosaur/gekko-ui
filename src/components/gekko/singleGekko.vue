  ...
  export default {
    ...
    methods: {
      ...
      restartGekko() {
        this.$q.dialog({
          title: 'Restart Gekko',
          message: 'Are you sure you wand to restart this Gekko?',
          ok: 'Yes',
          cancel: 'No'
        }).onOk(async () => {
          try {
            let stopRes = await this.$axios.post(`${this.$store.state.config.apiBaseUrl}restartGekko`, {id: this.data.id})
            this.$q.notify('Gekko has been restarted.')
          } catch (ex) {
            this.$q.notify('Error - could not restart Gekko.');
            console.log("Error stopping restart", ex);
          }
        }).onCancel(() => {
          return;
        })
